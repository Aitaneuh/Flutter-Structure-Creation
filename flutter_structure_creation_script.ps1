#récupérer l'environnement de flutter
...
@("include: package:flutter_lints/flutter.yaml", "", "linter:", "  rules:") | Add-Content -Path $path\package\domain_entities\analysis_options.yaml

New-Item -Name pubspec.yaml -ItemType file

@("name: domain_entities", "publish_to: none", "", "environment:", "  sdk: ^3.5.1", "", "dev_dependencies:","  flutter_lints: ^4.0.0") | Add-Content -Path $path\package\domain_entities\pubspec.yaml
...
foreach ($entity in $entities) {
    mkdir $entity
    Set-Location src\$entity

    New-Item -Name $entity.dart -ItemType file

    @("class $entity {", "  const $entity({", " })", "}") | Add-Content -Path $path\package\domain_entities\lib\src\$entity\$entity.dart
